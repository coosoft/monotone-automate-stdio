...
version (currently 0.99.1). If you happen to be using a newer version of
Monotone then this class will hopefully largely work but without the support
...
The one exception to the above is the $mtn-E<gt>generate_key() method, which
...
=item B<$mtn-E<gt>checkout(\@options, $ws_dir)>

Create a new workspace in the directory specified by $ws_dir. One can
...
The $options argument is a list of valid options, with some having
arguments. For example, one could call this method specifying all of the
options like this (feature: MTN_CONTENT_DIFF_EXTRA_OPTIONS):
...
=item B<$mtn-E<gt>drop_public_key($key_id)>
...
=item B<$mtn-E<gt>generate_key(\$buffer | \%hash, $key_id, $pass_phrase)>
...
=item B<$mtn-E<gt>get_extended_manifest_of(\$buffer | \@list, $revision_id)>
...
=item B<$mtn-E<gt>get_file_size(\$buffer, $file_id)>
...
=item B<$mtn-E<gt>get_public_key(\$buffer, $key_id)>
...
=item B<$mtn-E<gt>log(\@list[, \@options[, $file_name]])>
...
=item B<$mtn-E<gt>pull(\$buffer | \@list[,@options[, $uri]])>
...
=item B<$mtn-E<gt>push(\$buffer | \@list[,@options[, $uri]])>
...
=item B<$mtn-E<gt>put_public_key($public_key)>
...
=item B<$mtn-E<gt>sync(\$buffer | \@list[,@options[, $uri]])>
